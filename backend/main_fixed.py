...
<head>
    ...
    <title>{task.title() if task else 'Web Application'}</title>
    <style>
        body {{
...
<body>
    <div class="container">
        <h1>{task.title() if task else 'Task Manager'}</h1>
        <div class="task-form">
...
def generate_python_api_code(task):
    ...
    return {
        ...
        "code": f"""# FastAPI Application for {task.title()}
...
app = FastAPI(title="{task.title()} API", version="1.0.0")
...
@app.get("/")
async def root():
    return {{"message": "Welcome to {task.title()} API"}}
...
def generate_python_code(task):
    ...
    return {
        ...
        "code": f"""# Python Solution for {task.title()}
...
def generate_javascript_code(task):
    ...
    return {
        ...
        "code": f"""// JavaScript Solution for {task.title()}
...
def generate_data_science_code(task):
    ...
    return {
        ...
        "code": f"""# Data Science Solution for {task.title()}
...
class DataAnalyzer:
    ...
    def visualize_data(self):
        ...
        fig.suptitle('{task.title()} - Data Visualization', fontsize=16)
...
def generate_fallback_code(task):
    ...
    return {
        ...
        "code": f"""# Solution for {task.title()}
...
if __name__ == "__main__":
    ...
    print("Frontend will be available at: http://localhost:8001/static/index.html")
